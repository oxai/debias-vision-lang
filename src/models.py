...
VALID_FIT_MODELS = [
    f"m-bain/frozen-in-time/{x}" for x in ["ccwv2m", "wv2m", "ccwv2mcoco"]
]
VALID_SLIP_MODELS = [
    f"facebookresearch/SLIP/{x}"
    for x in ["ViT-B/CLIP", "ViT-B/SLIP", "ViT-L/CLIP", "ViT-L/SLIP"]
]
VALID_MODELS = (
    VALID_CLIP_MODELS + VALID_FIT_MODELS + VALID_SLIP_MODELS
)  # + VALID_FIT_MODELS, not included due to missing parse_config module
...
def model_loader(
    ...
    if model_name.startswith("openai/CLIP/"):
        ...
        if not os.path.isfile(os.path.join(PYTORCH_MODEL_CACHE, chkpt_fn)):
            subprocess.check_output(
                [
                    ...
                    "-P",
                    PYTORCH_MODEL_CACHE,
                    os.path.join(DOWNLOAD_URL, chkpt_fn),
                ]
            )
        ...
        checkpoint = torch.load(
            os.path.join(PYTORCH_MODEL_CACHE, chkpt_fn), map_location="cpu"
        )
        state_dict_fix = state_dict_data_parallel_fix(
...
class HuggingFaceTokenizerWrapper:
    ...
    def __call__(self, text, truncate=True):
        return self.tokenizer(
            text, return_tensors="pt", padding=True, truncation=truncate
        )
...
class FrozenInTime(nn.Module):
    def __init__(
        ...
        self.text_model = AutoModel.from_pretrained(
            text_params.get("model", "distilbert-base-uncased")
        )
...
if __name__ == "__main__":
    ...
    model, preprocess, tokenizer, alias_name = model_loader(
        "facebookresearch/SLIP/ViT-L/CLIP", "cuda"
    )
