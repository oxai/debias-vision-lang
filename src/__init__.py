...
PATHS = Dotdict()
PATHS.BASE = os.path.abspath(os.path.join(__file__, "..", ".."))
PATHS.DATA = os.path.abspath(os.path.join(PATHS.BASE, "datasets"))
PATHS.PLOTS = Dotdict()
...
PATHS.CACHE = os.path.abspath(os.path.join(PATHS.BASE, "cache"))

PATHS.MODEL_STORE = os.path.abspath(os.path.join(PATHS.BASE, "models"))

PATHS.TRAINED_MODELS = Dotdict()
PATHS.TRAINED_MODELS.BASE = os.path.join(PATHS.BASE, "models_trained")
PATHS.TRAINED_MODELS.METADATA = os.path.join(
    PATHS.TRAINED_MODELS.BASE, "runs_metadata.json"
)
PATHS.TRAINED_MODELS.TEST_PROMPTS = os.path.join(
    PATHS.TRAINED_MODELS.BASE, "test_prompts.json"
)
PATHS.TRAINED_MODELS.EMBEDDINGS = os.path.join(PATHS.TRAINED_MODELS.BASE, "embeddings")
...
PATHS.PSEUDOLABELS.GEN_BY = os.path.abspath(
    os.path.join(PATHS.PSEUDOLABELS.BASE, "generated_by.json")
)
PATHS.PSEUDOLABELS.DATA = os.path.abspath(os.path.join(PATHS.PSEUDOLABELS.BASE, "data"))
PATHS.PSEUDOLABELS.FAIRFACE = {
    mode: os.path.abspath(os.path.join(PATHS.PSEUDOLABELS.DATA, f"{mode}_FairFace"))
    for mode in ["train", "val"]
}
PATHS.PSEUDOLABELS.UTKFACE = os.path.abspath(
    os.path.join(PATHS.PSEUDOLABELS.DATA, f"UTKFace")
)

PATHS.UTKFACE = Dotdict()
PATHS.UTKFACE.BASE = os.path.abspath(os.path.join(PATHS.DATA, "utkface"))
PATHS.UTKFACE.IMGS = os.path.abspath(
    os.path.join(PATHS.UTKFACE.BASE, "data", "UTKFace")
)
...
PATHS.CELEBA.ATTRS = os.path.abspath(
    os.path.join(PATHS.CELEBA.DATA, "list_attr_celeba.txt")
)
...
PATHS.FOODI.RAW_METADATA = os.path.abspath(
    os.path.join(PATHS.FOODI.BASE, "raw_metadata.csv")
)
PATHS.FOODI.METADATA = os.path.abspath(os.path.join(PATHS.FOODI.BASE, "metadata.csv"))
PATHS.FOODI.EN_DESCS = os.path.abspath(
    os.path.join(PATHS.FOODI.BASE, "en_descriptions.pkl")
)
...
PATHS.FLICKR30K.METADATA = os.path.abspath(
    os.path.join(PATHS.FLICKR30K.BASE, "results_20130124.token")
)
PATHS.FLICKR30K.TEST_LIST = os.path.abspath(
    os.path.join(PATHS.FLICKR30K.BASE, "flickr30k_test.txt")
)
PATHS.FLICKR30K.IMAGES = os.path.abspath(
    os.path.join(PATHS.FLICKR30K.DATA, "flickr30k-images")
)
